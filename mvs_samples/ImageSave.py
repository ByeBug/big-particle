...
# 回调函数类型
ExceptionCallBackFuncType = CFUNCTYPE(None, c_uint, c_void_p)
...
def exception_callback(nMsgType, pUser):
    ...
    参数:
        nMsgType: unsigned int, 异常类型
        pUser: void*, 用户数据
    """
    user_data = None
    if pUser:
        user_data = cast(pUser, py_object).value
    
    print(f"Exception callback: {nMsgType}, {user_data}")

# c 函数指针
ExceptionCallBackFunc = ExceptionCallBackFuncType(exception_callback)
...
if __name__ == "__main__":
    ...
    try:
        ...
        device_info = cast(deviceList.pDeviceInfo[int(nConnectionNum)], POINTER(MV_CC_DEVICE_INFO)).contents
        ...
        # # 设置异常回调，回调函数指针这个参数有错误，暂时注释掉
        # user_data = {'hello': 'world'}
        # user_ptr = cast(py_object(user_data), c_void_p)
        # ret = cam.MV_CC_RegisterExceptionCallBack(ExceptionCallBackFunc, user_ptr)
        # if ret != 0:
        #     raise Exception("register exception callback fail! ret[0x%x]" % ret)

        # ch:探测网络最佳包大小(只对GigE相机有效) | en:Detection network optimal package size(It only works for the GigE camera)
        if device_info.nTLayerType == MV_GIGE_DEVICE or device_info.nTLayerType == MV_GENTL_GIGE_DEVICE:
            nPacketSize = cam.MV_CC_GetOptimalPacketSize()
            print("packet size: %d" % nPacketSize)
