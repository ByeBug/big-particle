...
def main():
    ...
    min_size = 50
    max_size = None
    start_time = '20250908 00:00:00'
    end_time = None
    
    print('警告: 将要删除以下条件的记录、OSS 记录和实际图片文件，此操作不可逆！')
    ...
    try:
        ...
        print(f"找到 {len(record_ids)} 条符合条件的记录")
        print(f"记录ID范围: {min(record_ids)} - {max(record_ids)}")
